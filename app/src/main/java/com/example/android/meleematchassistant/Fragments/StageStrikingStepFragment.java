...
public class StageStrikingStepFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        for (int i = 0; i < stageStrikingViewModel.stages.getStageList().length; i++) {
            ...
            CardView baseView = stageCardView.findViewById(R.id.base_view);
            baseView.setBackgroundResource(stageStrikingViewModel.stages.getStage(i).getStageImageResource());
            RadioButton baseViewRadioButton = baseView.findViewById(R.id.stage_button_layout_radioButton);
            baseViewRadioButton.setVisibility(View.GONE);
            final Drawable stageDrawable = baseView.getBackground();
            ...
            gameViewModel.getCurrentGame().observe(this, new Observer<Game>() {
                ...
                public void onChanged(@Nullable final Game currentGame) {
                    ...
                    if (((getActivity().getClass() == MainActivity.class && stageStrikingViewModel.stages.getStage(finalI).isStarterStage())
                            || getActivity().getClass() == StageSelectionActivity.class
                            && stageStrikingViewModel.stages.getStage(finalI).isStagePlayable())) {
                    ...
                    } else if (!stageStrikingViewModel.stages.getStage(finalI).isStarterStage()) { // If it's not a starter stage, just display a Toast message upon click
